...
public FileOutputStream(String name) throws FileNotFoundException {
    //其内部调用的是构造方法FileOutputStream(File file,boolean append)
    //其中append值默认设置为false
...
public FileOutputStream(String name, boolean append) throws FileNotFoundException{
    //其内部实现也是调用构造函数FileOutputStream(File file,boolean append)
...
```java
	...
	//其内部实现为调用方法writeBytes(byte b[],int off,int len,boolean append)
